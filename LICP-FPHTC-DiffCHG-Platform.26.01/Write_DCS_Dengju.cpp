...
int CDiffCHG::Write_DCS_Dengju()
{
	...
	for (k = 0; k < iNofStep_z; k++)
	{
		...
		ss.str("");
		ss.clear();
		filename = "CHG_DCS_Dengju" + temp + ".vasp";
		...
		for (j = 0; j < iNofStep_y; j++)
		{
			...
			{
				outall1 << ' ' << setprecision(12) << fDiffCHG_Sum_SunJunhui[i][j][k] * volume / pow(BOHR, 3);				
			}
			outall1 << '\n';
		}
		outall1.close();
	...
	char command[100];
	temp = "mkdir DCS_Dengju";
	strcpy(command, temp.c_str());
	system(command);
	temp = "mv CHG_DCS_Dengju* ./DCS_Dengju";
	strcpy(command, temp.c_str());
	system(command);
