...
int CInterface::Write_CS_Dengju()
{
	int i, j, k, l, m, n;
	...
	for (k = 0; k < iNofStep_z; k++)
	{
		...
		ss.str("");
		ss.clear();
		filename = "CHG_CS_Dengju_Interface" + temp + ".vasp";
		...
		for (j = 0; j < iNofStep_y; j++)
		{
			...
			{
				outall1 << ' ' << setprecision(12) << fCHG_Interface[i][j][k] * volume / pow(BOHR, 3);
			}
			outall1 << '\n';
			outall1.close();
		}
	...
	char command[100];
	temp = "mkdir CS_Dengju_Interface";
	strcpy(command, temp.c_str());
	system(command);
	temp = "mv CHG_CS_Dengju_Interface* ./CS_Dengju_Interface";
	strcpy(command, temp.c_str());
	system(command);
