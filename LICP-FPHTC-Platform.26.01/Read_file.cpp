...
int CInterface::Read_file(int jj)
{
	int i, j, k, l, m, n;
	...
	if (jj == 2)
	{
		...
		inall >> fStep_z_real_up;
                inall >> fDistance_x >> fDistance_y >> fDistance_z;
		...
		{
			cout << '\n';
			cout << "    The Nx, Ny or Nz contain illegal value. Please make sure 0<Nx<20, 0<Ny<20 and 0<Nz<40.";
			cout << '\n';
			cout << "    ";
			logout << '\n';
			logout << "    The Nx, Ny or Nz contain illegal value. Please make sure 0<Nx<20, 0<Ny<20 and 0<Nz<40.";
			logout << '\n';
			logout << "    ";
		...
		inall.close();
		inall.open("CHG_Interface.dat", ios::in);
		logout << "    CHG_Interface(e):   " << '\n';
		cout << "    CHG_Interface(e):   " << '\n';
		n = 0;
		for (i = 0; i < iNofStep_x; i++)
			for (j = 0; j < iNofStep_y; j++)
				for (k = 0; k < iNofStep_z; k++)
				{
					inall >> fCHG_Interface[i][j][k];
					n++;
					cout << "      " << setprecision(12) << fCHG_Interface[i][j][k] << ' ';
					logout << "      " << setprecision(12) << fCHG_Interface[i][j][k] << ' ';
					if (n % 5 == 0)
					{
						cout << '\n';
						logout << '\n';
		...
		inall.open("CHG_Interface_Center.dat", ios::in);
		logout << "    CHG_Interface_Center(e):   " << '\n';
		cout << "    CHG_Interface_Center(e):   " << '\n';
		n = 0;
		for (i = 0; i < iNofStep_x; i++)
			for (j = 0; j < iNofStep_y; j++)
				for (k = 0; k < iNofStep_z; k++)
				{
					inall >> fCHG_Interface_Center[i][j][k];
					n++;
					cout << "      " << setprecision(12) << fCHG_Interface_Center[i][j][k] << ' ';
					logout << "      " << setprecision(12) << fCHG_Interface_Center[i][j][k] << ' ';
					if (n % 5 == 0)
					{
						cout << '\n';
						logout << '\n';
