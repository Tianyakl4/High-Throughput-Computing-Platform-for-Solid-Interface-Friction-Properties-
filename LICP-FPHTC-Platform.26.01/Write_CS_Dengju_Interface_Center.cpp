...
int CInterface::Write_CS_Dengju_Interface_Center()
{
	int i, j, k, l, m, n;
	...
	for (k = 0; k < iNofStep_z; k++)
	{
		...
		ss.str("");
		ss.clear();
		filename = "CHG_CS_Dengju_Interface_Center" + temp + ".vasp";
		outall1.open(filename, ios::out);
		outall1 << "This file is  CS (e in VESTA) from LICP-FPHTC, and take the CHG format." << '\n';
		outall1 << "1.0" << '\n';
		...
		for (j = 0; j < iNofStep_y; j++)
		{
			...
			{
				outall1 << ' ' << setprecision(12) << fCHG_Interface_Center[i][j][k] * volume / pow(BOHR, 3);
			}
			outall1 << '\n';
		}
		outall1.close();
	...
	char command[100];
	temp = "mkdir CS_Dengju_Interface_Center";
	strcpy(command, temp.c_str());
	system(command);
	temp = "mv CHG_CS_Dengju_Interface_Center* ./CS_Dengju_Interface_Center";
	strcpy(command, temp.c_str());
	system(command);
