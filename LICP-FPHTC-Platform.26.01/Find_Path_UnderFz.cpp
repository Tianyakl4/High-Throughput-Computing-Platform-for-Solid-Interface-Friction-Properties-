...
int CInterface::Find_Path_UnderFz(double Fz)
{
	...
	else
	{
		int i;
		cout << '\n';
		cout << "    The load  " << Fz << " is beyond the calculation range. Input a random number to continue:";
		cout << '\n';
		cin >> i;
		return 1;
	}
...
void InitStack(SqStack& S)
{
	...
	if (!S.base) exit(EOVERFLOW);//存储分配失败
...
bool Pop(SqStack& S)
{
	...
	Node e;
	if (S.top == S.base)
		return false;//top=base表示为空栈，返回NULL
	else
	{
